...
local sRandomItem_1 = RandomInt( 1, 9 ) > 6 and "item_satanic" or "item_skadi"
...
local castUSDesire, castUSLocation = 0
...
function X.SkillsComplement()
  ...
  castUSDesire, castUSLocation = X.ConsiderUS()
	...
	then
		...
		bot:ActionQueue_UseAbilityOnLocation( abilityUS, castUSLocation )
		return
	end
...
function X.ConsiderQ()
	...
	then
		...
		if enemys[1] ~= nil and enemys[1]:IsAlive()
			...
			and ( not enemys[1]:HasModifier( "modifier_viper_poison_attack_slow" ) or J.IsGoingOnSomeone( bot ) )
...
function X.ConsiderUS()
    ...
    or not abilityUS:IsTrained()
    or not abilityUS:IsFullyCastable() then return 0 end

	local nRadius = abilityUS:GetSpecialValueInt( "radius" )
	local nCastRange = abilityUS:GetCastRange()
	local nCastPoint = abilityUS:GetCastPoint()
