...
local sRandomItem_1 = RandomInt( 1, 9 ) > 5 and "item_satanic" or "item_butterfly"
...
local nKeepMana, nMP, nHP, nLV, botTarget
local aetherRange = 0
...
function X.ConsiderQ()
	...
	--打架时先手
	if J.IsGoingOnSomeone( bot ) or J.IsChasingTarget( bot, botTarget )
	then
		if J.IsValidHero( botTarget )
			and J.CanCastOnNonMagicImmune( botTarget )
			and J.CanCastOnTargetAdvanced( botTarget )
			and J.IsInRange( botTarget, bot, nCastRange + 32 )
...
function X.ConsiderW()
  ...
  local nRadius = 200
  local nEnemysHerosInView = bot:GetNearbyHeroes( 1600, true, BOT_MODE_NONE )
	local nEnemysHerosNearby = bot:GetNearbyHeroes( 200, true, BOT_MODE_NONE )
  
  if #nEnemysHerosNearby >= 1 then
